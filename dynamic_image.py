...
if __name__ == "__main__":
    ...
    img_list = sorted(os.listdir(img_seq_dir_path),
                      key=lambda x: int(x.split('_')[-1].split('.')[0]))
    # should resize image to (image_size, image_size)
    img_pat_List = [os.path.join(img_seq_dir_path, img_name)
                    for img_name in img_list]
    images = [load_image(img_path) for img_path in img_pat_List if load_image(
        img_path) is not None]
    dynamic_image = get_dynamic_image(images)
    save_path = os.path.join(
        dest_path, img_seq_dir_path.split('/')[-2] + '.jpg')
