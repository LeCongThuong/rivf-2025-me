...
def make_balanced_loader(ds, batch_size=32, num_workers=4, balance=True):
    ...
    sampler = WeightedRandomSampler(
        sample_w.double(), num_samples=len(sample_w), replacement=True)
...
class CASMECSVDataset(Dataset):
    def __init__(
        ...
        self.transform = transform or build_transforms(
            grayscale=grayscale, train=True, target_size=target_size)
        self.drop_missing = drop_missing
        self.le = label_encoder

        # Clean labels (no mapping here — you said it's already done)

        labels_str = df["label"].astype(
            "string").str.strip().str.split().str[0].fillna("")
        samples, missing = [], 0
        for seq, lab in zip(df["Sequence"].astype(str), labels_str.astype(str)):
            ...
            if p is None:
                ...
                raise FileNotFoundError(
                    f"Image for {seq} not found in {self.images_dir}")
            samples.append((p, lab))
        if missing and self.drop_missing:
            print(
                f"[CASMECSVDataset] skipped {missing} rows (missing images).")

        self.samples = samples
        self.y = self.le.transform(
            [lab for _, lab in samples]).astype(np.int64)
...
def build_datasets_from_splits(
    ...
    le = LabelEncoder().fit(
        pd.concat([train_df["label"], valid_df["label"]], axis=0))

    train_tf = build_transforms(
        grayscale=grayscale, train=True,  target_size=target_size)
    valid_tf = build_transforms(
        grayscale=grayscale, train=False, target_size=target_size)
...
if __name__ == "__main__":
    ...
    ap = argparse.ArgumentParser(
        description="Build loaders from pre-split CSVs (sadness/fear already mapped).")
    ...
    ap.add_argument("--rgb", action="store_true",
                    help="Use RGB pipeline (default grayscale)")
    ap.add_argument("--no_balance", action="store_true",
                    help="Disable balanced sampler for train")
    args = ap.parse_args()
