...
def _build_valid_dataset(cfg: Config, classes_from_ckpt: list, CASMECSVDataset, build_transforms):
...
def _plot_confmat(cm: np.ndarray, class_names: list, out_png: Path, normalize: bool = False):
    ...
    if normalize:
        with np.errstate(all="ignore"):
            ...
            cm_to_show = np.divide(cm, row_sums, out=np.zeros_like(
...
def run_eval(cfg: Config):
    ...
    if classes is None:
        raise RuntimeError(
            "Checkpoint missing 'classes' list. Re-train and save classes in the state dict.")
    ...
    # 3) Build valid dataset/loader
    valid_ds = _build_valid_dataset(
        cfg,
        classes_from_ckpt=classes,
        CASMECSVDataset=CASMECSVDataset,
    ...
    if not all_true:
        raise RuntimeError(
            "No validation samples were loaded. Check your image paths/patterns.")
    ...
    # 6) Save artifacts
    _plot_confmat(cm, classes, out_png=outdir /
                  "confusion_matrix.png", normalize=False)
    _plot_confmat(cm, classes, out_png=outdir /
    ...
    print(
        f"Macro F1: {f1_macro:.4f} | Micro F1: {f1_micro:.4f} | Weighted F1: {f1_weight:.4f}")
    print(f"Per-class report saved to: {outdir/'classification_report.csv'}")
    print(
        f"Confusion matrix saved to: {outdir/'confusion_matrix.png'} (and normalized)")
