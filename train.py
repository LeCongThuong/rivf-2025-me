...
def set_deterministic(seed: int = 0) -> None:
    ...
    print(
        f"[deterministic] seed={seed} cudnn.deterministic=True cudnn.benchmark=False TF32=OFF")
...
def make_loaders_from_datasets(cfg: Config, train_ds, valid_ds, device: torch.device):
    ...
    # sampler can have its own stream
    gen_smpl = make_generator(cfg.seed + 123, on_cuda=(device.type == "cuda"))
...
def main(cfg: Config):
    ...
    train_loader, valid_loader = make_loaders_from_datasets(
        cfg, train_ds, valid_ds, device)
    if cfg.use_vector:
        extra_dim = 53
    ...
    if cfg.use_class_weights:
        ...
        class_w = compute_class_weights_from_data(
            y_train.numpy(), "inv_freq").to(device)
    ...
    scheduler = optim.lr_scheduler.CosineAnnealingLR(
        optimizer, T_max=cfg.epochs) if cfg.use_cosine else None
